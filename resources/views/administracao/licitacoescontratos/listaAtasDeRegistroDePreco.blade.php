@extends('administracao/licitacoescontratos.tabelaLiciCon')
                                ...
                                '<div class="col-md-12">'+
                                    '<table class="table table-sm">'+
                                        ...
                                        if(data.Arquivos.length > 0){
                                            ...
                                            '</thead>'+
                                            '<tbody>';
                                            @foreach($dadosDb3 as $valor)
                                                if(data.AtaID == {{ $valor->AtaID }}){
                                                    body = body + '<tr><td><a class="acessibilidade" target="_blank" href="{{ route("MostrarArquivoIntegra", ["permissao" => $valor->descricao, "SubCaminho" => $valor->SubCaminho, "ArquivoID" => $valor->ArquivoID]) }}" > {{ $valor->DescricaoArquivo }} </a></td></tr>';
                                                }
                                            @endforeach
                                            // $.each(data.Arquivos, function (key, arquivo) {
                                                // body = body + '<tr><td><a class="acessibilidade" target="_blank" href="{{ route("MostrarArquivoIntegra", ["permissao" => $valor->descricao, "SubCaminho" => $valor->SubCaminho, "ArquivoID" => $valor->ArquivoID]) }}" > {{ $valor->DescricaoArquivo }} </a></td></tr>';
                                                // body = body + '<tr><td><a class="acessibilidade" target="_blank" href="/arquivosintegra/exibirarquivo/' + arquivo.ArquivoID + '" >' + arquivo.DescricaoArquivo + '</a></td></tr>';
                                            // });
